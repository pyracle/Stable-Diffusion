...
class ResNetBlock(AbstractConfig):
    def __init__(self,
        ...
        self.sequential = keras.Sequential([
            convolutional_layer(256, (3, 3), padding='same'),
            keras.layers.BatchNormalization(),
            relu(),
            keras.layers.Dropout(dropout_rate),
            convolutional_layer(512, (3, 3), padding='same'),
            keras.layers.BatchNormalization(),
            relu(),
            pooling_or_up_sampling()
        ])
        self.skip = keras.Sequential([
            convolutional_layer(512, (1, 1), padding='same'),
            keras.layers.BatchNormalization(),
            relu(),
            pooling_or_up_sampling()
        ])
...
class AbstractSequential(AbstractConfig):
    def __init__(self,
        ...
        self.sequential = keras.Sequential([
            ResNetBlock(dropout_rate, convolutional_layer),
            keras.layers.Dropout(dropout_rate),
            ResNetBlock(dropout_rate, convolutional_layer),
            keras.layers.Dropout(dropout_rate),
